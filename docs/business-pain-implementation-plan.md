...
| 2 | Retention/revenue problems are noticed manually or by an isolated monitoring tool | The existing anomaly Lambda now runs a weekly mature-cohort D1/D7 retention check in addition to daily DAU/GGR; alerts enter the same SNS → Module 3 first-look path | Add an explainable novelty detector for previously unseen arbitrage only after reviewed labels and acceptable false-positive thresholds exist |
