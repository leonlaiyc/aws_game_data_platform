const translations = {
  zh: {
    ...
    storyTitle: "一個異常，如何從偵測走到結果驗證",
    storyIntro: "系統自動完成偵測與 first-look；深入調查、採取行動與驗證結果由人負責。實驗只是適用時的驗證方法，不是每個異常的固定下一步。",
    ...
    flowInvestigate: "深入調查",
    flowInvestigateBody: "分析師結合系統證據與營運脈絡，確認可能根因。",
    investigationEvidence: "Context · root cause · hypothesis",
    flowAct: "採取行動",
    flowActBody: "依根因執行營運修正、技術處置或候選產品變更。",
    actionOptions: "Operational fix · product change",
    humanLed: "人工主導",
    flowValidate: "驗證結果",
    flowValidateBody: "觀察 KPI 是否恢復；適合時才用 Module 2 做受控實驗。",
    validationOptions: "Monitor recovery · experiment if appropriate",
    flowScopeTitle: "這條閉環只描述 KPI／留存異常。",
    flowScopeBody: "規則式套利偵測已實作，但走 flagged players → human review 的獨立路徑，不會觸發 Module 3 first-look；未知手法偵測尚未宣稱。",
  ...
  en: {
    ...
    snapshotFoot: "ap-northeast-1 · Serverless-first · Synthetic data only",
    storyKicker: "ONE SYSTEM, NOT FOUR DEMOS",
    storyTitle: "How one anomaly moves from detection to a validated outcome",
    storyIntro: "The system automates detection and the first look. People own deeper investigation, action, and outcome validation. An experiment is one possible validation method—not the automatic next step.",
    ...
    flowInvestigateBody: "An analyst combines system evidence with operating context to identify likely root causes.",
    investigationEvidence: "Context · root cause · hypothesis",
    flowAct: "Take action",
    flowActBody: "Apply the appropriate operational fix, technical response, or candidate product change.",
    actionOptions: "Operational fix · product change",
    humanLed: "HUMAN-LED",
    flowValidate: "Validate the outcome",
    flowValidateBody: "Check whether KPIs recover; use Module 2 only when a controlled experiment is appropriate.",
    validationOptions: "Monitor recovery · experiment if appropriate",
    flowScopeTitle: "This loop covers KPI and retention anomalies only.",
    flowScopeBody: "The rule-based arbitrage detector is implemented, but follows a separate flagged players → human review path. It does not trigger Module 3 first-look, and no unknown-technique coverage is claimed.",
